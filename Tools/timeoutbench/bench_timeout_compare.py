...
def _timeout_check_func(implementation: str):
    import _timeout

    if implementation == "op":
        return _timeout._test_timeout_check
    if implementation == "ours":
        return _timeout.check
    raise RuntimeError("baseline has no _timeout check function")


def _configure_op_skip_interval(implementation: str, interval: int) -> None:
    if implementation != "op":
        return
    import _timeout

    _timeout._test_set_timeout_skip_interval(interval)


def bench_timeout_check_no_timeout(
    loops: int,
    implementation: str,
) -> float:
    if not _has_timeout(implementation):
        raise RuntimeError("baseline has no _timeout check benchmark")
    check = _timeout_check_func(implementation)
    t0 = pyperf.perf_counter()
    for _ in range(loops):
        check()
    return pyperf.perf_counter() - t0
...
def bench_timeout_check_active(
    loops: int,
    implementation: str,
    timeout_seconds: float,
    op_skip_interval: int,
) -> float:
    if not _has_timeout(implementation):
        raise RuntimeError("baseline has no _timeout check benchmark")
    _warm_timeout(implementation, timeout_seconds)
    check = _timeout_check_func(implementation)
    _configure_op_skip_interval(implementation, op_skip_interval)
    t0 = pyperf.perf_counter()
    with _get_timeout(timeout_seconds):
        _configure_op_skip_interval(implementation, op_skip_interval)
        for _ in range(loops):
...
def add_cmdline_args(cmd, args) -> None:
    ...
    cmd.extend(("--op-skip-interval", str(args.op_skip_interval)))
...
def main() -> None:
    ...
    runner.argparser.add_argument(
        ...
        type=int,
        default=16,
        help="skip interval used by the OP _test_timeout_check benchmark",
    )
    ...
    if _has_timeout(args.implementation):
        runner.bench_time_func(
            "_timeout_check_no_timeout",
            bench_timeout_check_no_timeout,
            args.implementation,
        )
        runner.bench_time_func(
            "_timeout_check_active",
            bench_timeout_check_active,
            args.implementation,
            args.timeout_seconds,
            args.op_skip_interval,
        )

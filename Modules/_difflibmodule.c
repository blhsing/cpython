...
chain_b(SequenceMatcherObject *self)
{
    ...
    int32_t *b_lbl = (int32_t *)PyMem_Malloc(
        sizeof(int32_t) * (size_t)(lb > 0 ? lb : 1));
...
_difflib_exec(PyObject *module)
{
    ...
    PyObject *match_name = PyUnicode_FromString("Match");
    PyObject *match_fields = PyUnicode_FromString("a b size");
    if (match_name == NULL || match_fields == NULL) {
        Py_XDECREF(match_name);
        Py_XDECREF(match_fields);
        Py_DECREF(namedtuple);
        return -1;
    }
    PyObject *match_args = PyTuple_Pack(2, match_name, match_fields);
    Py_DECREF(match_name);
    Py_DECREF(match_fields);
    if (match_args == NULL) {
        Py_DECREF(namedtuple);
        return -1;
    }
